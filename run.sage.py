...
def power_max_prime_fact(input):
	...
	if A % pref == _sage_const_0 :
		print 'time', int(time.time()-start_time), 'bestA', bestResult[_sage_const_1 ], 'bestOffset', bestResult[_sage_const_2 ]
...
def par():
	...
	out = zip(*pool.map(power_max_prime_fact, smart_search(_sage_const_27 )))
	result = min(zip(*out))
